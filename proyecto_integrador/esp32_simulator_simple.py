#!/usr/bin/env python3
"""
ESP32 Simulator (Versión simplificada) - Proyecto Integrador
...
class ESP32Simulator:
    ...
    def update_ldrs_to_firebase(self):
        ...
        if self.firebase_put("sensores/datos_" + str(timestamp), data):
            ...
            print("Error al enviar LDRs a Firebase")
    ...
    def check_control_from_firebase(self):
        ...
        try:
            ...
            # Obtener datos de control desde Firebase sin usar ordenamiento
            control_data = self.firebase_get("control")
            
            if control_data and isinstance(control_data, dict):
                # Buscar el control con el timestamp más alto
                highest_timestamp = 0
                latest_control = None
                ...
                for key, values in control_data.items():
                    if isinstance(values, dict) and 'timestamp' in values:
                        ...
                        if timestamp > highest_timestamp:
                            ...
                            latest_control = values
                
                # Si encontramos una entrada y es más reciente que nuestra última actualización
                if latest_control and highest_timestamp > last_update_time:
                    values_changed = False
                    
                    # Obtener los valores pwm para los 4 focos
                    for i in range(4):
                        key = f"pwm_value{i+1}"
                        if key in latest_control:
                            new_value = int(latest_control[key])
                    ...
                    if values_changed:
                        print(f"[{datetime.now().strftime('%H:%M:%S')}] Nuevos valores de control desde Firebase (timestamp: {highest_timestamp}):")
                        ...
                        last_update_time = highest_timestamp
            
        except Exception as e:
            print(f"Error en check_control_from_firebase: {e}")
            # Continuar funcionando incluso si hay errores en esta parte
    ...
    def simulate_sensor_changes(self):
        ...
        if self.light_pattern == "static":
            ...
            t = time.time()
            for i in range(4):
                phase = i * math.pi / 2  # Desplazamiento de fase para cada sensor
                sine_component = math.sin(t/10 + phase) * 200
                base_light = self.ambient_light * 1023
